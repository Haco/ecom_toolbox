...
/** Load extension list */
$GLOBALS['TCA']['sys_category']['columns']['tx_ext_type']['config']['items'][] = [ 'Local Extensions', '--div--' ];
foreach ( scandir(PATH_site . '/typo3conf/ext') as $directory ) {
	if ( !preg_match('/^\.\.?$/', $directory) ) {
		$GLOBALS['TCA']['sys_category']['columns']['tx_ext_type']['config']['items'][] = [ $directory, $directory ];
	}
}
